...
const LoginSchema = v.object({
  email: v.pipe(v.string('Email must be a string'), v.nonEmpty('Please enter your email'), v.email('Invalid email format')),
  password: v.pipe(
...
const LoginSchema = v.object({
  ...
  password: v.pipe(v.string(), v.nonEmpty('Please enter your password'), v.minLength(8, 'Password must be at least 8 characters')),
});
```
