...
import { createToken, getSession } from '@sfeir/helpers';
...
export const GET = async (request: NextRequest) => {
  const token = request.headers.get('authorization')?.replace('Bearer ', '') ?? request.cookies.get(AUTH_COOKIE_NAME)?.value;

  const payload = await getSession(token, AUTH_SECRET ?? '');
  if (!payload) {
